...
## Custom React tab items

To render arbitrary React components as tab items (Lottie, SVG, custom badges, full custom bar design), use `bottomTab.component` on **every** tab and optionally `bottomTabs.customRow` for the floating row chrome.

See the [Custom bottom tabs](customBottomTabs) guide for setup, props, platform differences, and examples.
