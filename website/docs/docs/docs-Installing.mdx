...
Use the toolchain from the React Native 0.87 template, including Node.js ≥ 22.13.0,
Android SDK / Build Tools 37, and Android Gradle Plugin 9. Keep the template's
compatibility settings in `android/gradle.properties`:
...
TypeScript projects must use a module resolver that understands package exports,
such as `moduleResolution: "bundler"` with `module: "esnext"`. The React Native
TypeScript configuration already provides this. React Native Navigation supports
the strict public types without opting into `react-native-legacy-deep-imports`.
...
> In iOS, from React-native 0.79 onwards the rnn-link will make the necessary changes only to the AppDelegate.swift file. The Objective-C version of the AppDelegate will need to be changed manually (see below).
...
-  [factory startReactNativeWithModuleName:@"RN79"
-                                      in:self.window
-                            launchOptions:launchOptions];
...
For the React Native 0.87 template, keep its `reactHost` property and package list.
Extend `NavigationApplication()` instead of `Application(), ReactApplication`, import
`com.reactnativenavigation.NavigationApplication`, and remove the `loadReactNative(this)`
call and its import: `NavigationApplication.onCreate()` initializes React Native.
There is no need to introduce a `reactNativeHost` property.

For applications that still declare `reactNativeHost`, use the following configuration:
