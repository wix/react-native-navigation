...
| `yarn test-all` | runs js, autolink, native unit, snapshot, and E2E tests; iOS unit and snapshot builds run sequentially because they share build output |

The iOS unit and snapshot runners accept `IOS_TEST_DEVICE` and `IOS_TEST_OS`. For example:

```sh
IOS_TEST_DEVICE="iPhone 17 Pro Max" IOS_TEST_OS="26.5" yarn test-unit-ios --release
```

Snapshot comparisons must use the same simulator model and OS as the reference images. `RECORD=true yarn test-snapshot-ios` records images locally only: review and commit intentional changes yourself. Recording never changes Git configuration, creates a remote, commits, or pushes, and test failures still produce a nonzero exit status.
