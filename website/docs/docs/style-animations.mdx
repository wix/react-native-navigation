...
Animating switches between roots, using enter/exit animation you can control how each root is animated towards entering or exiting the screen.
...
```
In Playground, you can try it by seting `useCustomAnimations` in `flags.ts` file.
