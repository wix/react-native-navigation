...
Works with [custom bottom tab items](../docs/docs-customBottomTabs.mdx) on iOS and Android.
...
Invoked when BottomTab is long pressed by a user. **iOS only.** Not emitted for [custom bottom tab items](../docs/docs-customBottomTabs.mdx) — long press is handled on the native tab bar, which is hidden when every tab uses `bottomTab.component`.
