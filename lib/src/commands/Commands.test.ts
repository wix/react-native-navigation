...
describe('Commands', () => {
    ...
    describe('updateProps', () => {
        ...
        it('notifies commands observer', () => {
            uut.updateProps('theComponentId', {someProp: 'someValue'});
            verify(commandsObserver.notify('updateProps', deepEqual({
    ...
    describe('switchToPIP', () => {
        it('switch a component to PIP mode, passing componentId', () => {
            ...
            verify(
                mockedNativeCommandsSender.switchToPIP('switchToPIP+UNIQUE_ID', 'theComponentId', deepEqual({}))
            ).called();
        });
        it('switch a component to PIP mode, passing componentId and options', () => {
            ...
            verify(mockedNativeCommandsSender.switchToPIP('switchToPIP+UNIQUE_ID', 'theComponentId', options)).called();
        });

        it('switchToPIP returns a promise that resolves to componentId', async () => {
            when(mockedNativeCommandsSender.switchToPIP(anyString(), anyString(), anything())).thenResolve(
                'theComponentId'
            );
    ...
    describe('notifies commandsObserver', () => {
        ...
        describe('passes correct params', () => {
            ...
            const paramsForMethodName: Record<string, object> = {
                ...
                switchToPIP: {commandId: 'switchToPIP+UNIQUE_ID', componentId: 'id', mergeOptions: {}},
                restorePIP: {commandId: 'restorePIP+UNIQUE_ID', componentId: 'id'},
                closePIP: {commandId: 'closePIP+UNIQUE_ID', componentId: 'id'},
                pushAsPIP: {commandId: 'pushAsPIP+UNIQUE_ID', componentId: 'id', layout: null}
            };
