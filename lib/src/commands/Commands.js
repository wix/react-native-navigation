...
class Commands {
  ...
  showOverlay(componentData, options) {
    const optionsInput = _.cloneDeep(options);
    OptionsProcessor.processOptions(optionsInput);
    const input = _.cloneDeep(componentData);
    const layout = this.layoutTreeParser.parse(input);
    this.layoutTreeCrawler.crawl(layout);

    const promise = this.nativeCommandsSender.showOverlay(layout, optionsInput);
