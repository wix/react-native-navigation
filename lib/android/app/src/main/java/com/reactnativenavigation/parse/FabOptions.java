...
public class FabOptions {
    public static FabOptions parse(JSONObject json) {
        ...
        if (json.has("actions")) {
            ...
            for (int i = 0; i < actions.length(); i++) {
                options.actionsArray.add(FabOptions.parse(actions.optJSONObject(i)));
            }
        }
    ...
    void mergeWith(final FabOptions other) {
        ...
        if (other.actionsArray.size() > 0) actionsArray = other.actionsArray;
    ...
    public FabOptions mergeWithDefault(FabOptions defaultOptions) {
        ...
        if (actionsArray.size() == 0) actionsArray = defaultOptions.actionsArray;
    ...
    public FabOptions copy() {
        FabOptions copy = new FabOptions();
        copy.mergeWith(this);
        copy.layoutId = layoutId;
        return copy;
    }
}
