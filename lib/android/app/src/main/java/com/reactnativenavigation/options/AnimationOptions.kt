...
open class AnimationOptions @JvmOverloads constructor(json: JSONObject? = null) : LayoutAnimation {
    ...
    fun getAnimation(view: View, defaultAnimation: Animator): Animator =
        if (hasAnimation()) {
            AnimatorSet().apply {
                playTogether(allPropertyAnimations(view))
            }
        } else {
            defaultAnimation
        }
    ...
    val duration: Int
        get() = CollectionUtils.reduce(propertyAnimOptions, 0, { item: ValueAnimationOptions, currentValue: Int -> max(item.duration[currentValue], currentValue) })

    open fun hasAnimation(): Boolean = propertyAnimOptions.isNotEmpty()

    fun isFadeAnimation(): Boolean = propertyAnimOptions.size == 1 && propertyAnimOptions.find(ValueAnimationOptions::isAlpha) != null

    fun setValueDy(animation: Property<View?, Float?>?, fromDelta: Float, toDelta: Float) {
        first(propertyAnimOptions, { o: ValueAnimationOptions -> o.equals(animation) }) { param: ValueAnimationOptions ->
            param.setFromDelta(fromDelta)
            param.setToDelta(toDelta)
        }
    }

    private fun allPropertyAnimations(view: View) =
        propertyAnimOptions.map { it.getAnimation(view) }

}

fun getAnimProp(key: String): Triple<Property<View, Float>, Int, (View) -> Float> {
    when (key) {
