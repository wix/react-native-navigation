...
open class OverlayTouchDelegate(
        private val component: TouchDelegateLayout,
        private val reactView: ReactView
) {
    var interceptTouchOutside: Bool = NullBool()

    fun onInterceptTouchEvent(event: MotionEvent): Boolean {
        return when (interceptTouchOutside.hasValue() &&
    ...
    @VisibleForTesting
    open fun handleDown(event: MotionEvent) =
            when (event.coordinatesInsideView(reactView.getChildAt(0))) {
